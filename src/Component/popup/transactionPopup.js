...
export default function TransactionPopup(props) {
  ...
  const selectToken = () => {
    let linkBSC = '';
    linkBSC = 'https://bscscan.com/tx/' + localStorage.getItem('transactionHash');
    let linkETH = '';
    linkETH = 'https://etherscan.io/tx/' + localStorage.getItem('transactionHash');
    const style = {
      color: 'white'
    };
    return <a style={style} href={props.chain == 'bsc' ? linkBSC : linkETH} target="_blank">{localStorage.getItem('transactionHash')}</a>;
  }
  const transactionLink = () => {
    ...
    return <a style={style} href={props.chain == 'bsc' ? linkBSC : linkETH} target="_blank">View transaction</a>
  }
  ...
  return (
    ...
    <Modal isOpen={props.isOpen} dismiss={props.dismiss} chain={props.chain}>
      ...
      <PPMainBx>
        ...
        <PPsBx01>
          <PPstitle01>
            Transaction Successfull &nbsp;<BsCheckLg />
...
const PPstitle01 = styled(FlexDiv)`
     align-items: flex-start; justify-content:flex-start; width:100%; font-size:14px; font-weight:700; padding:8px 0 12px 0; position:relative; color:#8be05a;
`;
